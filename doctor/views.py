...
def edit_prescription(request, appointment_id, prescription_id):
    ...
    if request.method =="POST":
        ...
        base_models.Prescription.objects.create(medications=medications)
...
def payments(request):
    doctor = doctor_models.Doctor.objects.get(user = request.user)
    payments = base_models.Billing.objects.filter(appointment_doctor=doctor, status ="Paid")
...
def profile(request):
    doctor = doctor_models.Doctor.objects.get(user= request.user)
    formatted_next_available_date=doctor.next_available_appointment_date.strftime('%Y-%m-%d')
    if request.method =="POST":
    ...
    context ={
        "doctor":doctor,
        "formatted_next_available_appointment_date" : formatted_next_available_date,
    }
